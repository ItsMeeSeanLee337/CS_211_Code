...
void DECODER(int *values, int n, int *iindex, int *oindex) {
    int s = 0;
    int i;
    for (i = 0; i < powd(2, n); i++) {
        values[oindex[i]] = 0;
    }
    for (i = 0; i < n; i++) {
        s += values[iindex[i]] * powd(2, n - i - 1);
    }
    values[oindex[s]] = 1;
}
void MUX(int *values, int n, int *iindex, int *sindex, int oindex) {
    ...
    int i;
    for (i = 0; i < n; i++) {
        s += values[sindex[i]] * powd(2, n - i - 1);
    }
...
int main(int argc, char** argv) {
    ...
    names[1] = "1\0";
    sum+=2;  // counter for names 0 and 1
    int i;
    sum += icount; // counter for icount below
    for (i = 0; i < icount; i++) {
    ...
    sum+=ocount; // counter for icount line 168
    for (i = 0; i < ocount; i++) {
